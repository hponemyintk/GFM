...
parser.add_argument("--train_stage", type=str, default="finetune",
                    choices=["finetune", "similarity_resample"])
...
if args.train_stage == "finetune":
    ...
    if local_rank == 0:
        print(f"[Finetune Stage] Training complete.")

elif args.train_stage == "similarity_resample":
    # Stage 2: Similarity-based resampling training
    best_val_metric = -math.inf if higher_is_better else math.inf
    state_dict = None
    ...
    # Final evaluation after similarity resampling:
    final_val_preds = test(loader_dict["val"], eval_model=model.module, epoch=0, desc="Val")
    final_test_preds = test(loader_dict["test"], eval_model=model.module, epoch=0, desc="Test")
    ...
    if local_rank == 0:
        ...
        file_path = os.path.join(output_path, str(args.seed) + "_stage2.json")
        with open(file_path, "w") as f:
            json.dump(best_metrics_dict, f, indent=4)

    if local_rank == 0:
        print(f"[Stage2] Similarity resample training complete.")
